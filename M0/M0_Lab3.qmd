...
format:
  html:
    ...
    fig-height: 5
categories: ['M0:', 'Lab']
description: "Hands-on lab for Module 0 focused on California Housing (MLP Regression) and Palmer Penguins (MLP Classification)."
---
...
> **Note**: For instructions on using Google Colab, Tensors, and Google Drive syncing via GitHub, please refer to [Module 0: Lecture Note 1 (M0_LN1.qmd)](./M0_LN1.qmd).
...
For each major section, include at least one validation output such as:

- dataset shape or class counts
- preprocessing confirmation
- training-loss trace
- evaluation metric table
- confusion matrix or diagnostic plot
...
This lab is the last structured-modeling checkpoint before the course moves into neural NLP. As you work, keep the following mapping in mind:

- numeric feature matrix here -> token or document matrix in NLP
- regression/classification head here -> text classification head later
- hidden layers here -> learned semantic transformation later
- train/validation/test discipline here -> the same generalization logic for text models later
...
For an introduction to the Google Colab environment, see [M0_LN1.qmd](M0_LN1.qmd).
