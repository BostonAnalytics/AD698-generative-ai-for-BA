...
//| echo: false
//| fig-align: "center"
//| fig-alt: "General NLP Pipeline"
//| caption: "General NLP Pipeline"
...
```{dot}
//| label: fig-sentiment-approaches 
//| echo: false
//| fig-align: "center"
//| fig-alt: "Sentiment Analysis Approaches"
...
```{dot}
//| label: fig-weblog-mining 
//| echo: false
//| fig-align: "center"
//| fig-alt: "Web Mining Process"
//| caption: "Web Mining Process"
...
```{dot}
//| label: fig-social-analytics 
//| echo: false
//| fig-align: center
//| fig-alt: "Social Analytics Overview"
//| caption: "Social Analytics Overview"
...
//| echo: false
//| fig-align: center
//| fig-alt: "NLP Tasks Overview"
//| caption: "NLP Tasks Overview"
digraph NLP_Tasks {
...
//| echo: false
//| fig-align: "center"
//| fig-alt: "Prompting Example"
//| caption: "Prompting Example"
digraph PromptLogic {
  rankdir=LR
  node [shape=plaintext]
...
//| echo: false
//| fig-align: "center"
//| fig-alt: "Fine-tuning Example"
//| caption: "Fine-tuning Example"
...
//| echo: false
//| fig-align: "center"
//| fig-alt: "Data Split Example"
//| caption: "Data Split Example"
digraph DataSplit {
  rankdir=LR
  node [shape=plaintext]
...
```{dot}
//| label: fig-summarization-pipeline
//| echo: false
//| fig-align: "center"
//| fig-alt: "Summarization Pipeline"
...
![Classical vs. Deep Learning NLP](./M01_lecture02_figures/classical_nlp_and_deep_learning_based_nlp_layouts.jpg){width=80% fig-align="center" #fig-nlp-layouts fig-alt="Classical vs. Deep Learning NLP"}
...
Bag of Words counts every word equally.
But common words like *the*, *is*, and *and* appear everywhere and carry little discriminating information.

**TF-IDF** (Term Frequency–Inverse Document Frequency) solves this by down-weighting terms that appear in many documents.
...
where $f_{t,d}$ is the count of term $t$ in document $d$, and $N$ is the total number of documents.

Words that are **frequent in one document** but **rare across the corpus** receive high TF-IDF weight — these are the words that best characterize that document.
...
corpus = [
    ...
    "operating costs declined due to restructuring",
    "the company reported record revenue this year",
    "restructuring charges impacted fourth quarter results",
]
...
tfidf_df.style.hide(axis="index")
...
| Common word handling | Counts them equally | Penalizes common words |
| Unique word emphasis | None | Boosts discriminating terms |
| Interpretability | High | High |
| Best for | Baselines, frequency tasks | Classification, similarity |

Both representations produce **sparse, fixed-length vectors** — they are the foundation before learning dense embeddings.


# Sentiment Analysis with a PyTorch MLP

## From Features to Neural Classification

Once text is represented as TF-IDF vectors, we can feed those vectors directly into a neural network.
This bridges **classical NLP feature engineering** and **deep learning**.

The architecture is a simple **Multi-Layer Perceptron (MLP)**:
...
where $x$ is the TF-IDF vector for a document.
...
# ── Tiny labeled corpus ──────────────────────────────────────
texts = [
    "revenue increased profit rose earnings beat expectations",
    "strong growth quarterly results exceeded analyst forecasts",
    "record sales driven by product demand and market expansion",
    "operating costs declined margins improved efficiency gains",
    "loss widened charges impacted results below expectations",
    "restructuring charges write-downs impaired assets quarter",
    "revenue declined sales dropped demand weakened significantly",
    "net loss reported guidance cut outlook revised downward",
]
...
Beyond accuracy, text classifiers are evaluated with **precision**, **recall**, and **F1**:
...
| Era | Representation | Model | Strength |
|---|---|---|---|
| Classical | BoW / TF-IDF | Naïve Bayes, SVM | Fast, interpretable |
| Transition | Word2Vec embeddings | Shallow MLP | Semantic similarity |
| Deep Learning | Contextual embeddings | RNN, LSTM | Sequence-aware |
| Modern | Transformer embeddings | BERT, GPT | Full-context, generative |

Each row builds on the previous.
The MLP above sits at the **transition row** — it uses classical features (TF-IDF) but learns nonlinear patterns through a neural network.
...
//| echo: false
//| fig-align: "center"
//| fig-alt: "Sentiment Pipeline"
//| caption: "Sentiment Pipeline"
...
## What's Next: From Representations to Language Models

The representations developed in this lecture — BoW, TF-IDF, and MLP classifiers — treat text as *unordered* feature bags.
Words have individual weights, but sequence and context remain invisible.

In Module 2 we formalize **language as a probability distribution over sequences**.
That shift — from classifying documents to predicting the next token — lays the mathematical foundation for everything from N-gram language models to the transformer-based architectures that power modern generative AI.
