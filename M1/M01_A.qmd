...
1. **Word Document (`.docx`)** — maximum 5 pages (excluding cover page and references)
...
Use the SEC EDGAR submissions API to retrieve the filing index for each company, then download Item 1A (Risk Factors) and Item 7 (MD&A) for years 2020–2024.
...
For each filing, extract Item 1A and Item 7 text. Store each extracted section as one row in a corpus DataFrame:
...
For each year (2020–2024), compute cosine similarity between the mean TF-IDF vector for Company A's Item 1A and Company B's Item 1A. Repeat for Item 7.
...
📊 **Required:** Line chart of cosine similarity over years, with Item 1A and Item 7 as separate lines. Label axes and include a legend.
...
Retrain your PyTorch BoW classifier separately on Item 1A chunks only and on Item 7 chunks only. Report accuracy for each run.
...
Submit AI disclosure as a separate document or as an appendix to your Word submission.
