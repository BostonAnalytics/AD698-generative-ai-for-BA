...
format: 
    revealjs:
        ...
        transition: fade
        auto-stretch: false
        footer: "AD 698 - Early Neural Networks and NLP"
...
categories: ['NLP Foundations', 'SEC Text', 'Language Probability', 'Text Representation', 'Conceptual Framing']
description: "Introduces Early Neural Networks and NLP through presentation-ready concepts, technical intuition, diagrams, and applied connections to the course analytics workflow."
---

# Welcome to AD698 - Applied Generative AI
...
![](./M01_lecture01_figures/ad698path.png){width="80%"}
...
course_schedule.drop("Description", axis=1, inplace=True)
...
## Course Grading Table {.compact-table}
...
- Use **Google Colab** as the primary notebook runtime
- Save notebooks, reports, and artifacts in **Google Drive**
- Submit deliverables through the course workflow, not through a required GitHub repository
- Include the required report and **AI disclosure document** alongside the notebook when assigned
- Keep one reproducible notebook per assignment or project milestone whenever possible

This course is built around evidence, reproducibility, and clean deliverables rather than repository management.
...
- The project is completed through staged milestones, a final managerial report with application demo, a presentation, and group feedback.
- Milestones are intended to build progressively rather than appear all at once at the end of the semester.

| **Component**                     | **Points** | **Description**                                                                                                                                    |
|-----------------------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------------|
| Managerial report with application demo | 70   | Includes milestone progression across corpus familiarization and scope, text structuring and retrieval units, embeddings and retrieval baseline, and grounded generation with guardrails |
...
- [OpenAI API]{.uured-bold}: access to GPT models for generation and fine-tuning.
- [LangChain]{.uured-bold}: framework for building LLM-powered applications and agents.
- [Cohere]{.uured-bold}: alternative LLM provider with strong NLP capabilities. 
- [Google Colab]{.uured-bold}: free cloud notebooks with GPU support for development and experimentation.
...
## Simulation, Machine Learning, and AI
...
:::
::: {.column width="40%"}
![](./M01_lecture01_figures/dartmouth_Fathers_AI.png){width="100%" fig-align="center"}
:::
::::
...
#| code-fold: true
#| fig-align: center
#| fig-cap: "The XOR problem: a simple example of a non-linearly separable function that the original perceptron could not learn."
...
for ax, (name, y) in zip(axes, gates.items()):
    ...
    for (x1, x2), label in zip(X, y):
        if label == 0:
            ax.scatter(x1, x2, c="#766100", marker="o", s=80)
        else:
            ax.scatter(x1, x2, c="#3e4587", marker="D", s=80)
    ...
    if name in ["AND", "NAND", "OR"]:
        ...
        if name == "AND":
            ...
            ax.plot(xs, 1.5 - xs, color="#766100")
        elif name == "NAND":
            # Example: x1 + x2 = 0.5
            ax.plot(xs, 0.5 - xs, color="#766100")
        elif name == "OR":
            # Example: x1 + x2 = 0.5
            ax.plot(xs, 0.5 - xs, color="#3e4587")
...
$$
\begin{align}
% Weights and activations in a neural network
...
:::
::: {.compass-card}
[What the theorem does (and does not) say]{.card-heading}
...
:::: {.compass-cards}
::: {.compass-card}
[What AlexNet showed]{.card-heading}
...
:::
::: {.compass-card}
[Why ImageNet mattered]{.card-heading}
...
:::
::: {.compass-card}
[Infrastructure innovation]{.card-heading}
...
:::
::: {.compass-card}
[Why this mattered]{.card-heading}
...
:::
::: {.compass-card}
[From models to platforms]{.card-heading}
...
- **NLP** is the subfield of AI concerned with enabling computers to understand, interpret, and generate human language
- Text is **unstructured**: no rows, no columns, no fixed schema
- Computers need numbers — NLP provides the bridge
...
| Task | Example |
|------|---------|
| Tokenization | "great product" → ["great", "product"] |
| Classification | Review → Positive/Negative |
| Named Entity Recognition | "Apple" → ORG |
| Summarization | Article → Key points |
:::
:::{.column width="45%"}
**Why it matters for business**

- Analyze customer reviews at scale
- Extract signals from SEC filings
...
- Vocabulary $V$ maps every unique word to a dimension
...
- **Term Frequency – Inverse Document Frequency** rewards words that are *distinctive*, not just common
- Stopwords ("the", "and") get low scores; domain-specific terms get high scores

$$
\begin{align}
\text{TF-IDF}(t, d) = \underbrace{\frac{\text{count}(t, d)}{|d|}}_{\text{TF}} \times \underbrace{\log \frac{N}{1 + \text{df}(t)}}_{\text{IDF}}
\end{align}
$$

> **Intuition**: "Revenue" appearing only in financial docs → high IDF. "The" appearing everywhere → low IDF.
...
- One of the most widely deployed NLP tasks in industry
...
| Text | Sentiment |
|------|-----------|
| "Exceptional product quality" | Positive |
...
Early methods: hand-crafted **lexicons** (VADER, SentiWordNet).  
Modern approach: **neural networks trained end-to-end** on labeled examples.
...
$$
\begin{align}
\text{Raw Text} \xrightarrow{\text{TF-IDF}} \mathbf{x} \xrightarrow{\text{MLP}} \hat{y}
\end{align}
$$

:::: {.columns}
::: {.column width="55%"}
![](./M01_lecture01_figures/MLP.png){width="100%" fig-align="center"}
:::
::: {.column width="45%"}
**Architecture**
...
## Softmax and Cross-Entropy in NLP

Why do softmax and cross-entropy matter so much here?

- softmax converts vocabulary or class scores into probabilities
- cross-entropy rewards the model for assigning high probability to the correct target

These are the same training mechanics behind both:
...
## Early Neural Language Model Intuition

The core language-modeling task is:
...
- representing context words numerically
- transforming them through learned layers
- producing logits over the vocabulary
- applying softmax

This is the bridge from classical NLP to modern LLMs.

---

## From Classical NLP to Deep Learning

| Era | Method | Strength | Weakness |
|-----|--------|----------|----------|
| 1990s | BoW + Naive Bayes | Simple, fast | Ignores order |
| 2000s | TF-IDF + SVM | Strong baseline | Manual features |
| 2010s | MLP / CNN / RNN | Learns features | Needs labeled data |
| 2017+ | Transformers (BERT, GPT) | Context-aware | High compute cost |

> **Key insight**: Each era builds on the last. TF-IDF vectors are still used to initialize or fine-tune modern models in low-resource settings.

---

## Generalization and Domain Shift

Text models can overfit or fail under domain shift just like other neural systems.

This matters in AD698 because later work moves from generic text into:

- business documents
- SEC filings
- domain-specific vocabulary

So accuracy on one text source is never the whole story.
