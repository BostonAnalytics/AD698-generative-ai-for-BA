...
import sys
sys.path.append("../utils")   # so Quarto/Jupyter can find diagram.py
...
# CREATE DIAGRAM
# ---------------------------------------------------------
d = Diagram("./M01_lecture02_figures/nlp_pipeline.svg", width=1400, height=900)

# ---------------------------------------------------------
# CLUSTERS
# ---------------------------------------------------------
d.cluster("Lexical processing", x=60, y=60, w=260, h=380, fill="#e8f0ff")
d.cluster("Structural representation", x=380, y=120, w=320, h=380, fill="#fff4d6")
d.cluster("Algorithm", x=780, y=120, w=520, h=320, fill="#D6EBD6")

# ---------------------------------------------------------
# NODES
# ---------------------------------------------------------
# Lexical
d.node("Characters", 100, 120)
d.node("Tokens", 100, 230)
d.node("Tagged tokens", 100, 340)

# Structural
d.node("Syntax tree", 430, 180, fill="#fff4d6", stroke="#e9bc55")
d.node("Entity relationships", 430, 290, fill="#fff4d6", stroke="#e9bc55")
d.node("Knowledge base", 430, 400, fill="#fff4d6", stroke="#e9bc55")

# Algorithm (2×2)
d.node("Regular expression", 820, 170, fill="#73AF59", stroke="#99CC99", text_color="#fff")
d.node("Part-of-Speech", 1100, 170, fill="#73AF59", stroke="#99CC99", text_color="#fff")
d.node("Logic compiler", 820, 300, fill="#73AF59", stroke="#99CC99", text_color="#fff")
d.node("Information extractor", 1100, 300, fill="#73AF59", stroke="#99CC99", text_color="#fff")

# ---------------------------------------------------------
# DATA FLOW ARROWS (solid blue)
# ---------------------------------------------------------
d.arrow("Characters", "Tokens",
 label="data flow"
 )
d.arrow("Tokens", "Tagged tokens",
label="data flow"
)

d.arrow("Syntax tree", "Entity relationships",
label="data flow"
)
d.arrow("Entity relationships", "Knowledge base",
label="data flow"
)

d.arrow("Tokens", "Syntax tree", label="data flow")
d.arrow("Tagged tokens", "Syntax tree"
, label="data flow"
)

# ---------------------------------------------------------
# ALGORITHMIC INFLUENCE (dashed green, higher curves)
# ---------------------------------------------------------
d.arrow("Regular expression", "Tokens",
        # label="algorithmic influence",
        color="#73AF59", dashed=True, algorithmic=True)

d.arrow("Regular expression", "Tagged tokens",
        # label="algorithmic influence",
        color="#73AF59", dashed=True, algorithmic=True)

d.arrow("Syntax tree", "Part-of-Speech",
        # label="algorithmic influence",
        color="#73AF59", dashed=True, algorithmic=True)

d.arrow("Knowledge base", "Logic compiler",
        # label="algorithmic influence",
        color="#73AF59", dashed=True, algorithmic=True)

d.arrow("Knowledge base", "Information extractor",
        # label="algorithmic influence",
        color="#73AF59", dashed=True, algorithmic=True)
