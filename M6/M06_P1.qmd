...
format: 
    revealjs:
        ...
        transition: fade
        auto-stretch: false
        footer: "AD 698 - Agentic AI Foundations"
...
categories: ['Agentic AI', 'Planning', 'Tools', 'Governance', 'Conceptual Framing']
description: "Introduces Agentic AI Foundations through presentation-ready concepts, technical intuition, diagrams, and applied connections to the course analytics workflow."
---
...
- [AI Agents:]{.uured-bold} Capable of autonomous operation. They are given a high-level goal, and they independently break it down into steps, execute them, and verify the results.
- Agents operate with a "loop" of perception and action, whereas assistants operate on a linear input/output mechanism.

## What makes a System "Agentic"?
- [Autonomy:]{.uured-bold} The ability to execute multi-step tasks without human intervention.
- [Contextual Memory:]{.uured-bold} The ability to store, retrieve, and learn from past interactions.
- [Tool Use:]{.uured-bold} The ability to interface with external APIs, databases, and filesystems to affect the real world.
- [Dynamic Routing:]{.uured-bold} The ability to decide *which* tool or sub-agent to use based on real-time environmental feedback.

## Agency, Autonomy, and Autonomicity

| Concept | Core Question | Example |
|---|---|---|
| Agency | Can the system pursue a goal through action? | Retrieve evidence, draft an answer, and check it |
| Autonomy | How independently can it operate? | Run a multi-step analysis without step-by-step approval |
| Autonomicity | Can it regulate itself? | Detect failure, retry, revise, or ask for help |

- A useful agent should not merely act.
- It should monitor whether its actions are working.

## Levels of Agency in Module 6

| Level | Description | Course Example |
|---|---|---|
| Assistive | Human drives the workflow | summarize retrieved SEC chunks |
| Tool-using | Bounded tools are available | retrieve Item 7 passages |
| Workflow-agentic | Multi-step execution inside constraints | plan, retrieve, answer, and log evidence |
| Multi-agent | Specialized roles coordinate | researcher, analyst, critic, compliance reviewer |

- More agency means more need for constraints, traces, and review.
...
- Modern agents repeat this loop until they stop, delegate, fail, or ask for help.
- Architecture determines what controls each transition.
...
- \(\mathcal{S}\): possible states of the environment
- \(\mathcal{A}\): possible actions
- \(P(s_{t+1}\mid s_t,a_t)\): transition dynamics
- \(R(s_t,a_t)\): reward or utility
...
- A POMDP agent acts under uncertainty.
- The agent maintains a probability distribution over possible hidden states.
- Modern LLM agents are not literal POMDP solvers, but the language helps us reason about partial context and uncertainty.
...
- A **skill** is reusable procedural knowledge.
- Tools answer: *what can the system call?*
- Skills answer: *how should the system perform a class of work?*
- Skills can include:
  - instructions,
  - examples,
  - templates,
...
![Agent skills architecture showing skills, MCP, subagents, hooks, tools, filesystem context, and plugins.](M06_lecture02_figures/agent_skills_architecture.jpg){width=80% fig-align=center}

## What Each Layer Provides

| Component | What It Provides | SEC Agent Example |
|---|---|---|
| Tool | executable capability | retrieve SEC evidence |
| Skill | reusable procedure | compare risk disclosures |
| Memory | durable context | ticker mappings and prior evidence |
| Policy | action selection rule | retrieve before answering |
| Trace | process record | tool calls, outputs, source rows |

## MCP Servers Are Not Agents

| Concept | Primary Role | Typical Unit |
|---|---|---|
| MCP server | capability boundary | tool, resource, data connector |
| Tool | executable action | function or API call |
| Skill | procedural knowledge | reusable workflow |
| Agent | control policy | loop over state, actions, observations |
| Multi-agent system | coordinated control | role-specialized agents |

- MCP answers: **How can the agent connect to this capability?**
- Agentic AI answers: **How should the system act over time?**
...
- Updating an agent involves versioning its agent instructions, tool schemas, and underlying base model simultaneously.
...
|---|---|---|
| User input | malicious or ambiguous requests | intent checks and policy gates |
| Retrieved content | prompt injection or false evidence | treat content as evidence, not authority |
| Tool calls | unsafe action or invalid parameters | typed schemas and permissions |
| Memory | stale or sensitive context | retention policy and review |
| Final answer | unsupported claims | citation and faithfulness checks |

## Evidence Is Not Instruction

- Retrieved content can support an answer.
- Retrieved content should not control the agent.
- This distinction matters when agents read:
  - web pages,
  - emails,
  - code repositories,
  - documents,
  - and SEC filings.
- A robust agent separates:
  - **instructions** from trusted system or course context,
  - **evidence** from retrieved materials,
  - **actions** from bounded tools.
