...
execute:
    ...
    warning: false
    message: false
categories: ['Tokenization', 'Embeddings', 'Semantic Search', 'Technical Foundations', 'Semantic Geometry']
description: "Explains Tokenization, Embeddings, and Semantic Geometry with the technical concepts, mathematical intuition, implementation patterns, and evaluation mindset needed for applied GenAI analytics."
---
...
# Reasoning Models Still Depend on the Basics

Modern reasoning models can appear to plan, explain, and correct themselves, but those behaviors still depend on the basic mechanics covered in this lecture. A step-by-step answer is still a sequence of generated tokens. The model still has to represent the current context, compute attention over available tokens, and choose likely continuations. Post-training and prompting can make the generated sequence look more like a careful solution path, but the underlying substrate remains language modeling.

This is why tokenization, embeddings, and attention matter for reasoning. Tokenization determines the units the model can condition on. Embeddings determine how tokens and concepts are represented geometrically. Attention determines which parts of the context influence the next prediction. Later modules add retrieval, reranking, evaluation, and agents, but those systems all build on these same representational foundations.
...
Keep two short sentences in mind while reading the tensor shapes:

| Sentence | Possible task |
|---|---|
| The team ships the model before lunch. | Predict the next token or label each token. |
| The analyst reviews quarterly revenue trends. | Build contextual representations for business text. |

Both are only six or seven words long, but attention still creates a full token-by-token
relationship matrix. That matrix is what lets the model connect *model* with *ships*, or
*revenue* with *trends*, without waiting for a recurrent state to carry the information
forward.
