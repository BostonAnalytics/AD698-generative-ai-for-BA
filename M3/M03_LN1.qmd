...
execute:
    ...
    warning: false
    message: false
categories: ['Sequence Models', 'RNN', 'Transformer', 'Conceptual Foundations', 'Transformers']
description: "Explains Sequence Models and Transformers with the technical concepts, mathematical intuition, implementation patterns, and evaluation mindset needed for applied GenAI analytics."
---
...
# Reasoning Models as Motivation

Before going into sequence models, it helps to start with the application behavior students now see in modern LLM products: reasoning models that appear to plan, solve problems step by step, revise mistakes, and call tools. A practical overview is provided by Labellerr's article ["LLMs & Reasoning Models: How They Work and Are Trained"](https://www.labellerr.com/blog/llms-reasoning-models-how-they-work/), which frames reasoning as more than recalling facts. In that framing, reasoning means transforming information across steps, making logical connections, using cause-and-effect structure, and sometimes delegating work to external tools.

This is a useful entry point for Module 3 because it makes prompting concrete. A reasoning model does not stop being a language model. It is still producing text token by token. What changes is the surrounding system design: prompts that ask for decomposition, examples that demonstrate stepwise solutions, self-consistency strategies that compare multiple paths, tool calls for search or calculation, and post-training feedback that rewards better answers or better intermediate processes.

For business analytics, this distinction matters. A model that merely sounds fluent is not enough for financial or operational analysis. We need systems that can show evidence, explain steps, admit uncertainty, and revise when new information appears. The rest of this lecture works backward from that goal: first understanding prompts as inference-time control signals, then understanding the sequence models and transformer mechanisms that make prompt-conditioned generation possible.
...
Consider two deliberately small translation examples:

| Source | Target | Alignment intuition |
|---|---|---|
| The small robot opens the red door. | Der kleine Roboter öffnet die rote Tür. | Local adjective-noun structure mostly stays aligned. |
| The analyst reviews the quarterly revenue report. | La analista revisa el informe trimestral de ingresos. | A compact English noun phrase becomes a longer Spanish phrase. |

The key teaching point is that the decoder should not receive the same source summary at
every output step. When producing *Roboter*, it should focus on *robot*; when producing
*de ingresos*, it may need to attend to the phrase *revenue report* rather than a single
word. This is the intuition behind attention as soft alignment.
...
The help-code transformer notebooks break this down into two useful visual pieces. First,
scaled dot-product attention computes a weighted combination of values from query-key
similarities:

![Scaled dot-product attention](./M03_lecture01_figures/transformer-attention.png){width=70% fig-align=center fig-alt="Scaled dot-product attention diagram with query, key, value, softmax, and output." #fig-ln-transformer-attention}

Second, multi-head attention repeats this operation in several learned subspaces, then
combines the heads:

![Multi-head attention](./M03_lecture01_figures/transformer-mha-01.png){width=72% fig-align=center fig-alt="Multi-head attention diagram showing multiple attention heads and output projection." #fig-ln-transformer-mha}
