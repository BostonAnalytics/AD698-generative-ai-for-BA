...
format: 
    revealjs:
        ...
        transition: fade
        auto-stretch: false
        footer: "AD 698 - Transformer Mechanics and Semantic Geometry"
...
categories: ['Prompt Engineering', 'Tokenization', 'Embeddings', 'Semantic Search', 'Technical Deep Dive']
description: "Introduces Transformer Mechanics and Semantic Geometry through presentation-ready concepts, technical intuition, diagrams, and applied connections to the course analytics workflow."
---
...
# Reasoning Models Still Start Here

## From "Show Your Work" to Token Prediction

Reasoning models often appear to solve problems by planning, reflecting, and correcting themselves.

Under the hood, they still rely on the same foundation:

- text is split into tokens
- the model predicts likely next tokens
- attention determines which context tokens matter
- training and post-training shape the kinds of sequences the model prefers

The course claim for today:

> Reasoning is built on language modeling, not separate from it.

## What Training Adds to Basic Generation

Reasoning-oriented systems improve basic generation through:

- pretraining on code, math, explanations, and structured arguments
- instruction tuning on step-by-step tasks
- examples that teach decomposition patterns
- feedback loops that reward useful or correct outputs
- tool-use patterns that delegate search or calculation

This helps explain why we study the basics:

tokenization, sequence probability, embeddings, and attention are the substrate on which richer reasoning behaviors are built.

## Guiding Question for Lecture 2

When a model gives a step-by-step answer, each step is still generated as text.

So we need to understand:

1. how the model represents the current context
2. how it scores the next token
3. why attention changes what information is available
4. how embeddings make similarity and retrieval possible

Once those pieces are clear, later modules can add retrieval, evaluation, and agentic tool use.
...
Language generation is therefore a probabilistic unfolding of tokens through context.
...
- A smaller value means:
  - less surprise
  - better prediction
  - more accurate modeling of language structure.
...
[Perplexity]{.uured-bold} is a common metric for evaluating language models. It is defined as the exponential of the average cross-entropy:
...
That is why later LLM systems require additional control layers such as instruction tuning and Reinforcement Learning from Human Feedback (RLHF).
...
## Fixed-size Images and Word Windows
...
![The same function and parameters applied to three sequence inputs](M03_lecture02_figures/shared-parameters.svg){width=95%}
...
![Each successive network receives a growing list of previous inputs](M03_lecture02_figures/all-previous-inputs.svg){width=90%}
...
![Different input sizes require different functions at successive steps](M03_lecture02_figures/growing-input-functions.svg){width=90%}
...
![Inputs, hidden states, and outputs connected across three recurrent steps](M03_lecture02_figures/recurrent-state.svg){width=95%}
...
## Recurrent State and Shared Parameters
...
![The previous state and current input produce the updated state](M03_lecture02_figures/compact-recurrence.svg){width=70%}
...
![Recurrent states carry information across sequence prediction steps](M03_lecture02_figures/recurrent-sequence-tasks.svg){width=92%}
...
## Tiny Sequence Examples

| Task | Input | Output |
|---|---|---|
| Next-token prediction | The team ships the model before | lunch |
| Sequence labeling | The analyst reviews quarterly revenue trends | DET NOUN VERB ADJ NOUN NOUN |
| Translation | The small robot opens the red door | Der kleine Roboter öffnet die rote Tür |

- These are deliberately short so every token can be inspected.
- The model sees a sequence, maintains or retrieves context, and produces either a token, a label sequence, or another sentence.
- RNNs carry context through state; transformers retrieve context through attention.
...
> prompt writing → prompt optimization
...
## Comparing Parameter Update Strategies
