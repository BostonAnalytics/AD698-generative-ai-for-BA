...
course_schedule.drop([14, 15, 16, 17], axis=0, inplace=True)

# Identify the last two columns (assumed to be dates)
date_cols = course_schedule.columns[-2:]

# On-campus (BU-aware, break-aware)
oncampus_df, oncampus_lecture_dates = build(return_dates=True)


semester_start = oncampus_lecture_dates[0]

self_learn_date = semester_start - timedelta(days=7)

# Online (calendar-agnostic, 7 consecutive weeks)
...
# Expand to 14 lectures by pairing
online_lecture_dates = []
for d in online_dates:
...
# Self-learn (rows 0 and 1)
course_schedule.loc[0:1, ["Online", "On-Campus"]] = self_learn_date

# On-campus lectures (rows 2–15 → 14 lectures)
course_schedule.loc[2:15, "On-Campus"] = pd.Series(oncampus_lecture_dates)

# Online paired lectures (rows 2–15 → 14 lectures)
course_schedule.loc[2:15, "Online"] = pd.Series(online_lecture_dates)
