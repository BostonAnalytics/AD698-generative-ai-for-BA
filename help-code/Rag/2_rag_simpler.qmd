---
title: 2 - Multi-Year Tesla RAG with Reranking
jupyter: python3
execute:
  eval: false
  echo: true
...
# Multi-Year Tesla RAG with Reranking

This notebook starts from the baseline Tesla 10-K RAG pipeline and adds retrieval improvement strategies.

The main comparison is:

1. **similarity retrieval**: nearest chunks by embedding similarity
2. **MMR retrieval**: diverse chunks selected through Maximal Marginal Relevance
3. **optional Cohere reranking**: an external reranker that reorders candidate chunks when `COHERE_API_KEY` is available

## Setup

```{python}
%pip install -q beautifulsoup4 requests pandas faiss-cpu sentence-transformers langchain langchain-core langchain-classic langchain-community langchain-text-splitters langchain-huggingface langchain-openai cohere langchain-cohere
```
...
mmr_retriever = vector_store.as_retriever(
    search_type="mmr",
    search_kwargs={"k": 8, "fetch_k": 32, "lambda_mult": 0.45},
)
...
if "COHERE_API_KEY" in os.environ:
    ...
    retrievers["cohere_rerank"] = ContextualCompressionRetriever(
        base_retriever=similarity_retriever,
        base_compressor=cohere_reranker,
    )
else:
    print("COHERE_API_KEY is not set. Skipping optional Cohere reranker.")
```
...
retrieval_compare_df = pd.DataFrame(retrieval_rows)
retrieval_compare_df
```

```{python}
retrieval_compare_df.groupby("retriever").agg(
...
## Generate Answers for Each Retriever
...
prompt = ChatPromptTemplate.from_messages([
    (
        ...
        (
            ...
            "Compare filing years when the question asks about change over time. "
...
rerank_answer_df = pd.DataFrame(answer_rows)
rerank_answer_df[["qid", "retriever", "response_mode", "retrieved_chunk_ids", "response"]]
```

## Save Reranking Artifacts

```{python}
output_dir, drive_output_dir = get_output_dirs()

retrieval_compare_df.to_csv(output_dir / "tesla_reranking_retrieval_compare.csv", index=False)
rerank_answer_df.to_csv(output_dir / "tesla_reranking_answer_compare.csv", index=False)
...
Similarity retrieval often works well for focused questions. MMR can be better for multi-year or cross-topic questions because it retrieves a more diverse set of chunks. A reranker can improve the ordering of candidates when the initial vector search retrieves a noisy top-k set.
