---
title: "4 - Tool-Style Multi-Year Tesla RAG"
jupyter: python3
execute:
  eval: false
  echo: true
...
# Tool-Style Multi-Year Tesla RAG

This notebook turns retrieval into explicit tools. Instead of hiding retrieval inside a single chain, each tool exposes a specific evidence operation:

- retrieve across all Tesla filing years
- retrieve within one target year
- retrieve early-period evidence
- retrieve recent-period evidence

Tool-style RAG is useful when a question needs controlled evidence gathering before synthesis.

## Setup

```{python}
%pip install -q beautifulsoup4 requests pandas faiss-cpu sentence-transformers langchain langchain-community langchain-text-splitters langchain-huggingface langchain-openai
```
...
all_years_store = FAISS.from_documents(chunks, embedding_model)
all_years_retriever = all_years_store.as_retriever(search_type="mmr", search_kwargs={"k": 6, "fetch_k": 30})

chunks_by_year = {}
stores_by_year = {}
retrievers_by_year = {}

for year in sorted({doc.metadata["filing_year"] for doc in chunks}):
    year_chunks = [doc for doc in chunks if doc.metadata["filing_year"] == year]
    chunks_by_year[year] = year_chunks
    stores_by_year[year] = FAISS.from_documents(year_chunks, embedding_model)
    retrievers_by_year[year] = stores_by_year[year].as_retriever(search_kwargs={"k": 4})
```

## Retrieval Tools

```{python}
def docs_to_tool_text(docs, max_chars_per_doc: int = 700) -> str:
    return "\n\n".join(
        f"[TSLA {doc.metadata['filing_year']} {doc.metadata['chunk_id']}]\n{doc.page_content[:max_chars_per_doc]}"
        for doc in docs
    )

@tool
def search_all_tesla_10k_years(query: str) -> str:
    """Search all Tesla 10-K filing years for evidence related to the query."""
    docs_for_q = all_years_retriever.invoke(query)
    return docs_to_tool_text(docs_for_q)

@tool
def search_tesla_10k_year(query_and_year: str) -> str:
    """Search one Tesla 10-K year. Input format: 'YEAR | query'."""
    try:
        year_text, query = query_and_year.split("|", 1)
        year = int(year_text.strip())
    except Exception:
        return "Input must look like: 2021 | battery supply risk"
    if year not in retrievers_by_year:
        return f"No cached retriever for year {year}."
    return docs_to_tool_text(retrievers_by_year[year].invoke(query.strip()))

@tool
def search_early_tesla_10k_period(query: str) -> str:
    """Search Tesla 10-K filings from 2015 through 2018."""
    early_chunks = [doc for year in range(2015, 2019) for doc in chunks_by_year.get(year, [])]
    store = FAISS.from_documents(early_chunks, embedding_model)
    docs_for_q = store.as_retriever(search_kwargs={"k": 6}).invoke(query)
    return docs_to_tool_text(docs_for_q)

@tool
def search_recent_tesla_10k_period(query: str) -> str:
    """Search Tesla 10-K filings from 2022 through 2025."""
    recent_chunks = [doc for year in range(2022, 2026) for doc in chunks_by_year.get(year, [])]
    store = FAISS.from_documents(recent_chunks, embedding_model)
    docs_for_q = store.as_retriever(search_kwargs={"k": 6}).invoke(query)
    return docs_to_tool_text(docs_for_q)

tools = [
    search_all_tesla_10k_years,
    search_tesla_10k_year,
    search_early_tesla_10k_period,
    search_recent_tesla_10k_period,
]
```

## Manual Tool Plan

The manual tool plan keeps the notebook runnable without an agent framework. It calls targeted tools and then synthesizes over the gathered evidence.

```{python}
question = TESLA_EVAL_QUESTIONS[1]["user_input"]

tool_evidence = {
    "early_period": search_early_tesla_10k_period.invoke(question),
    "recent_period": search_recent_tesla_10k_period.invoke(question),
    "all_years": search_all_tesla_10k_years.invoke(question),
}

pd.DataFrame(
    {"tool": name, "evidence_preview": text[:500]}
    for name, text in tool_evidence.items()
)
```

## Synthesize Tool Evidence
...
synthesis_prompt = ChatPromptTemplate.from_messages([
    (
        "system",
        (
            "Synthesize the retrieved Tesla 10-K evidence. "
            "Compare early and recent filings when relevant. "
            "Cite years and chunk IDs found in the evidence."
        ),
    ),
    ("user", "Question:\n{question}\n\nTool evidence:\n{evidence}"),
])

evidence_text = "\n\n".join(f"## {name}\n{text}" for name, text in tool_evidence.items())

if "OPENAI_API_KEY" in os.environ:
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
    answer_chain = synthesis_prompt | llm | StrOutputParser()
    tool_answer = answer_chain.invoke({"question": question, "evidence": evidence_text})
else:
    tool_answer = evidence_text
...
The manual plan above is usually enough for teaching. When an agent library is available, the same tools can be bound to a tool-calling model.
...
pd.DataFrame(
    {"question": question, "tool": name, "evidence": text}
    for name, text in tool_evidence.items()
).to_csv(output_dir / "tesla_tool_rag_evidence.csv", index=False)
