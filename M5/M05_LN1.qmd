...
RETRO++ [@RETROpp] & Pre-training Corpus& Text& Chunk& Pre-training&Iterative\\
...
| TriviaQA (TQA) [@TQA] | [@DenseX;@CoN;@FILCO;@GenRead;@ICRALM;@InstructRetro]<br/>[@ITRG;@LLM-boundary;@RA-DIT;@RAG;@RAG-Integration]<br/>[@RAVEN;@RECITE;@RECOMP;@Replug;@RETROpp;@self-rag] |
...
## Scaling laws of RAG
End-to-end RAG models and pre-trained models based on RAG are still one of the focuses of current researchers [@RAFT] .The parameters of these models are one of the key factors.While scaling laws [@kaplan2020scaling]  are established for LLMs, their applicability to RAG remains uncertain. Initial studies like RETRO++ [@RETROpp]  have begun to address this, yet the parameter count in RAG models still lags behind that of LLMs. The possibility of an Inverse Scaling Law~\footnote{\url{https://github.com/inverse-scaling/prize}}, where smaller models outperform larger ones, is particularly intriguing and merits further investigation.
...
While scaling laws [@kaplan2020scaling]  are established for LLMs, their applicability to RAG remains uncertain. Initial studies [@RETROpp]  have begun to address this, yet the parameter count in RAG models still lags behind that of LLMs. The possibility of an Inverse Scaling Law\footnote{\url{https://github.com/inverse-scaling/prize}}, where smaller models outperform larger ones, is particularly intriguing and merits further investigation.
