...
categories: ['RAG', 'LangChain Splitters', 'Chunking', 'Vector Search', 'HNSW', 'Conceptual Foundations']
description: "Explains RAG Foundations and Retrieval with the technical concepts, mathematical intuition, implementation patterns, and evaluation mindset needed for applied GenAI analytics."
---
...
```python
%pip install -U langchain-core langchain-text-splitters pandas
```
...
A useful practitioner framing is that a foundation model is a reusable base, while a RAG application is a system wrapped around that base. The Cycle.io Learn Center describes this distinction in approachable terms: foundation models provide broad pretrained capability, but RAG adds a retriever and external knowledge store so the model can consult domain evidence at query time.[^cycle-rag] This is exactly the distinction students should keep in mind for SEC analysis: the language model supplies fluent synthesis, while the retrieval system supplies auditable filing evidence.

[^cycle-rag]: Cycle.io Learn Center, ["Foundation Models and RAG"](https://cycle.io/learn/foundation-models-and-retrieval-augmented-generation).
...
For a 10-K question such as "What evidence supports Apple's AI investment narrative?", a prompt-only answer can only draw from the model's learned memory and whatever appears in the prompt. A RAG answer should retrieve passages from the filing, preserve metadata such as company, year, item, and chunk ID, and then generate a response that cites the retrieved evidence. This side-by-side contrast is the simplest way to explain why RAG is an architecture, not just a longer prompt.
...
Chunking requires decisions about document structure, metadata, and which elements must remain together. LangChain's HTML splitters differ in how they use headings, retain metadata, and preserve semantic elements such as tables and lists.
...
The story map in @fig-hnsw-story-map illustrates navigation from broad regions to local detail. Early steps in the search are broad, exploratory, and coarse. Later steps become local, specific, and refinement-oriented. That is exactly what the hierarchy is doing: upper layers help us move across the space quickly, while lower layers help us make precise nearest-neighbor decisions.
...
![Detailed story-beat analogy for layered HNSW search.](./M05_lecture01_figures/hsw_starwars_harrypotter_detailed.png){width=82% fig-align="center" #fig-hsw-story-detailed fig-alt="Detailed Star Wars and Harry Potter analogy mapped through HNSW layers."}
...
![Voronoi-style partition of vector neighborhoods below HNSW layers.](./M05_lecture01_figures/hsw_voronoi_general.png){width=74% fig-align="center" #fig-hsw-voronoi-neighborhoods fig-alt="Voronoi-style partition of vector space under HNSW layers."}
...
![Conceptual HNSW search for books similar to *Dune*.](./M05_lecture01_figures/hsw_dune.png){width=82% fig-align="center" #fig-hsw-dune-search fig-alt="Conceptual HNSW search for books similar to Dune."}
...
![HNSW layers and navigation intuition.](./M05_lecture01_figures/hnsw_1_explained.png){width=72% fig-align="center" #fig-hnsw-layers-navigation fig-alt="HNSW layers and navigation intuition."}
...
![HNSW search process.](./M05_lecture01_figures/hnsw_2_search.png){width=72% fig-align="center" #fig-hnsw-search-process fig-alt="HNSW search process."}
...
![HNSW insertion process.](./M05_lecture01_figures/hnsw_3_insertion.png){width=72% fig-align="center" #fig-hnsw-insertion-process fig-alt="HNSW insertion process."}
