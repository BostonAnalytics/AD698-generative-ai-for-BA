...
execute:
  echo: true
  eval: false
categories: ['RAG', 'SEC 10-K', 'Evaluation', 'Auditability', 'Applied Assessment', 'Business Analytics']
description: "Assesses students' ability to apply Prompting, Retrieval, and Evaluation Methods for SEC 10-K Analysis through reproducible analysis, evidence-based reasoning, validation, and business interpretation."
---
...
:::{.callout-note}
This assignment is intentionally more hands-off than the labs. The tutorials and labs should help you understand retrieval and evaluation mechanics, but the final comparison workflow, prompts, and interpretation should be your own.
:::
...
1. **Word Report (`.docx`)** — maximum 10 pages, APA 7 formatted
   - All 9 LLM answers (3 questions x 3 methods) as a formatted table or appendix
   - 3 x 3 evaluation matrix
...
5. Run all 9 answers (3 questions x 3 methods)
6. Build and fill the 3 x 3 evaluation matrix
...
ðŸ“Š **Required:** Stacked bar chart of chunk counts by company and by year.
...
# Build the 3 x 3 Evaluation Matrix {#sec-eval}
...
Fill in all `None` values manually by reading each answer and the corresponding retrieved chunks. This is the **3 x 3 evaluation matrix** required in your Word document.

**Grounding score calculation (for RAG answers):**

Count the number of sentences in the RAG answer that explicitly reference a filing year, item (e.g., "Item 7", "Item 7A", or "Item 8"), or company name divided by the total sentence count.
...
**Answer in your Word document (<=1 page):** Based on your 3 x 3 evaluation matrix, which of the three methods produces the most auditable answers? Your answer must reference:
...
| 3 x 3 evaluation matrix (`eval_df`) | In notebook + Word doc |
...
![ChatGPT shared conversation link interface](../M0/M0_lecture01_figures/chatgpt-share.jpg){width="80%" fig-align="center" fig-alt="ChatGPT shared conversation link interface" #fig-a-chatgpt-share}

![Gemini shared conversation link interface](../M0/M0_lecture01_figures/gemini-share-conversation.png){width="80%" fig-align="center" fig-alt="Gemini shared conversation link interface" #fig-a-gemini-share}
