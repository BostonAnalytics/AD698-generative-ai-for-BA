...
:::{.callout-warning}
**API key security:** Store your API key in Colab Secrets (left panel â†’ key icon) or in a `.env` file on Drive. Never paste a key directly into a code cell. Never commit a key to GitHub.
:::
...
1. **Word Report (`.docx`)** — maximum 10 pages, APA 7 formatted
   - All 9 LLM answers (3 questions Ã— 3 methods) as a formatted table or appendix
   - 3Ã—3 evaluation matrix
...
5. Run all 9 answers (3 questions Ã— 3 methods)
6. Build and fill the 3Ã—3 evaluation matrix
...
ðŸ“Š **Required:** Display all 9 answers as a formatted table in your notebook (one row per question, three answer columns). Truncate to 200 characters per cell if needed for readability.

# Build the 3Ã—3 Evaluation Matrix {#sec-eval}
...
Fill in all `None` values manually by reading each answer and the corresponding retrieved chunks. This is the **3Ã—3 evaluation matrix** required in your Word document.
...
**Answer in your Word document:** Identify the question where RAG produces the most noticeably better answer than zero-shot prompting. Show the two answers side by side (â‰¤3 sentences each). Show `.head()` of the retrieved chunks that enabled the RAG answer. Explain in 2–3 sentences why the retrieved context mattered — what specific information did the chunks provide that the zero-shot answer lacked?
...
**Answer in your Word document (â‰¤1 page):** Based on your 3Ã—3 evaluation matrix, which of the three methods produces the most auditable answers? Your answer must reference:
...
| 3Ã—3 evaluation matrix (`eval_df`) | In notebook + Word doc |
